...
- 공지 목록(최신순, 제목+시간) → `/notices/[id]` 상세. 참가자는 읽기 전용
...
로그인 실패 시 팀 번호는 유지하고 비밀번호만 지웁니다. 빨간 배너 `Sign in failed. Check the details you provided are correct.` 와 빨간 입력 테두리가 표시됩니다. 성공하면 **대진표**(`/`)의 **우리팀** 세그먼트로 이동합니다. 대진표 탭은 공지로 바뀌지 않습니다.
